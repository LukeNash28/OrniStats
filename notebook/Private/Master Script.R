...
OrniStatsLocations = data.frame(read_excel("/Users/lukenash1/Documents/OrniStats/Public/Rare Bird Data - Main.xlsx", sheet = "Location Co-ordinates"))
...
recordTable <- function(qualifier, path = NULL) {
  require(dplyr)
  require(glue)
  require(kableExtra)
  
  subData = data.frame(subset(OrniStatsRecords, Qualifier == qualifier & "Year of Discovery" != 2023))
...
yearlyForecast <- function(qualifier, path=NA) {
  ...
  subData = data.frame(subset(OrniStatsRecords, Qualifier == qualifier & 
...
speciesPlots <- function(qualifier, path = NA) {
  ...
  combinedData <- subset(combinedData, !is.na(Date.of.Discovery))
  combinedData <- subset(combinedData, !is.na(Date.of.Departure))
  
  stayCounts <- combinedData %>%
  ...
  stayPlotYearly <- ggplot(combinedData, aes(Year.of.Discovery, Stay.Length))+
  ...
  stayPlotLat <- ggplot(combinedData, aes(lat, Stay.Length))+
    geom_point(color = "red", size=2)+
    geom_smooth(se=FALSE, method="loess", color="black", linewidth=1.5)+
    theme_classic()+
  ...
  stayPlotLong <- ggplot(combinedData, aes(long, Stay.Length))+
    geom_point(color = "red", size=2)+
    geom_smooth(se=FALSE, method="loess", color="black", linewidth=1.5)+
    theme_classic()+
  ...
  combinedData <- combinedData[!is.na(combinedData$Date.of.Discovery) 
                               & !is.na(combinedData$Date.of.Departure), ]
  
  arrivalPlotYearly <- ggplot(combinedData, aes(Year.of.Discovery, yday(Date.of.Discovery)))+
    geom_point(color = "red", size=2)+
    theme_classic()+
  ...
  arrivalPlotLat <- ggplot(combinedData, aes(yearday, lat))+
    geom_point(color = "red", size=2)+
    geom_smooth(se=FALSE, method="loess", color="black", linewidth=1.5)+
    theme_classic()+
  ...
  arrivalPlotLong <- ggplot(combinedData, aes(yearday, abs(long)))+
    geom_point(color = "red", size=2)+
    geom_smooth(se=FALSE, method="loess", color="black", linewidth=1.5)+
    theme_classic()+
...
speciesRegression <- function(qualifier, x, y, max_degree = 10, omit_dates = FALSE) {
  require(dplyr)
  require(ggplot2)
  require(DescTools)
  require(semEff)
